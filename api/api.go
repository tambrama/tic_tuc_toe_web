...
import (
	...
	dsDto "tic-tac-toe/internal/datasource/dto"

	// webDto "tic-tac-toe/internal/web/dto"
	dsMappers "tic-tac-toe/internal/datasource/mappers"
...
func (api *GameAPI) HandlerMakeMove(w http.ResponseWriter, r *http.Request) {
	...
	currentGame := dsMappers.FromDB(gameDTO)
	...
	if currentStatus != services.InProgress {
		response := webMappers.FromDomain(currentGame, currentStatus)
	...
	response := webMappers.FromDomain(updateGame, newStatus)
...
func (api *GameAPI) HandlerNewGame(w http.ResponseWriter, r *http.Request) {
	...
	response := webMappers.FromDomain(newGame, services.InProgress)
